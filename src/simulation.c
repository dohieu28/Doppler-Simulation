...
int run_simulation(ModulationType mod_type, const char *output_file)
{
    ...
    uint8_t *tx_bits = malloc(num_data_syms * bits_per_symbol * sizeof(uint8_t));
    uint8_t *rx_bits = malloc(num_data_syms * bits_per_symbol * sizeof(uint8_t));
    ...
    for (int snr = 0; snr <= 30; snr += 2)
    {
        ...
        {
            // 1. Phía phát
            generate_random_bits(tx_bits, num_data_syms * bits_per_symbol);
            switch (mod_type)
            {
            case MODQPSK:
                modulate_qpsk(tx_bits, tx_qam, num_data_syms);
                break;
            case MODD16QAM:
                modulate_16qam(tx_bits, tx_qam, num_data_syms);
                break;
            case MODD64QAM:
                modulate_64qam(tx_bits, tx_qam, num_data_syms);
                break;
            }
            ...
            {
            case MODQPSK:
                demodulate_qpsk_hard(rx_qam, rx_bits, num_data_syms);
                break;
            case MODD16QAM:
                demodulate_16qam_hard(rx_qam, rx_bits, num_data_syms);
                break;
            case MODD64QAM:
                demodulate_64qam_hard(rx_qam, rx_bits, num_data_syms);
                break;
            }

            // 7. (Tùy chọn) Tính tỷ lệ lỗi bit BER cho 1 vòng lặp
            int bit_errors = 0;
            int total_bits = num_data_syms * bits_per_symbol;
